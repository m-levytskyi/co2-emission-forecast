...
def prepare_features(df):
    ...
    feature_cols = [
        col for col in df.columns if col not in ["timestamp", "value", "state", "type"]
    ]
...
def train_xgboost_model(X, y, test_size=0.2, random_state=42):
    ...
    with mlflow.start_run():
        ...
        mlflow.xgboost.log_model(
            model, "model", registered_model_name="co2-intensity-xgboost"
        )
        ...
        return model, {"test_mae": test_mae, "test_rmse": test_rmse, "test_r2": test_r2}
...
def cross_validate_model(X, y, n_splits=5):
    ...
    with mlflow.start_run():
        ...
        mlflow.log_metrics({"cv_mean_mae": cv_mean, "cv_std_mae": cv_std})
